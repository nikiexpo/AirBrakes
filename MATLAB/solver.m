...
function [t, y] = solver(func, tspan, y0, h)
...
for n = 1:(length(t_rk)-1)
    k1 = feval(func,t_rk(n), y(n,:));
    %disp(size(k1))
    k2 = feval(func,t_rk(n)+h/2, y(n,:)+h/2.*k1);
    k3 = feval(func, t_rk(n)+h/2, y(n,:)+h/2.*k2);
    k4 = feval(func,t_rk(n+1), y(n,:)+h.*k3);
