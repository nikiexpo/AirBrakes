    ...
    methods
        function obj = rocketDynamics()
            if nargin == 0
                ...
                obj.dragCoeffABon = 0.79;
                obj.dragCoeffABoff = 0.35;
                ...
                obj.position = [0, 0, 232]';
                obj.quaternion = [1, 0, 0, 0]';
                obj.linearVelocity = [0, 0, 166]';
                obj.angularVelocity = [0.3, 0, 2]';
        ...
        function [obj] = update(obj,dt)
            obj.position = obj.position + r_dot(obj).*dt;
            obj.quaternion = quatmultiply(obj.quaternion', q_dot(obj)');
            obj.quaternion = obj.quaternion ./ norm(obj.quaternion);
            obj.linearVelocity = obj.linearVelocity + v_dot(obj).*dt; % VELOCITY DOESNT CHANGE
            obj.angularVelocity = obj.angularVelocity + omega_dot(obj).*dt;
        end
...
%         end
        ...
        function [q] = q_dot(obj)
            ...
            imag_part = q_w.*omega + cross(omega', q_v')';
            scalar_part = 0.5.*(dot(omega, q_v));
            q = [scalar_part, 0.5.*(imag_part)']';
        end
        function [v] = v_dot(obj)
            ...
            C_A_rocket = obj.dragCoeffABon / sqrt(1 - (norm(obj.linearVelocity)/a)^2 ); %compressibility compensated axial drag coeff
            ...
            AoA = acos(dot(norm_V_app, e_roll)); % SHOULD THIS BE RADIANS or degs???
        ...
        function omega = omega_dot(obj)
            ...
            R = quat2rotm(obj.quaternion'); % THIS IS NAN SEE WHY? -- not NAN after changing the omega intial conditions
            I = obj.diagonalInertia;
...
%% TODo
% The whole thing is unstable -- suspect the integration method, do RK4
% impelement the remaining dynamics -- not the variations just the
% deterministic ones